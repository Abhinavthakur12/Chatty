...
const adminLogin = TryCatch(async(req,res,next)=>{
    ...
    res.status(200).cookie("chatty-admin-token",token,{...cookieOptions,maxAge:1000*60*15}).json({
        success:true,
...
const adminLogout = TryCatch(async(req,res,next)=>{
    res.status(200).cookie("chatty-admin-token","",{...cookieOptions,maxAge:0}).json({
        success:true,
...
const getDashboardStats = TryCatch(async(req,res)=>{
    console.log("request to a rhe hai bhai>>...")
