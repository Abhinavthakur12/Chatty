...
const addMembers = TryCatch(async(req,res,next)=>{
    ...
    if (!chat.creator) return next(new ErrorHandler("Chat creator not found", 400));
    ...
    if(chat.members.length>100){
        return next(new ErrorHandler("Maximum 100 members allowed",400))
    }
    await chat.save()
...
const removeMember = TryCatch(async(req,res,next)=>{
    ...
    if(chat.members.length <= 3) return next(new ErrorHandler("Group Atleat have 3 members"))
    ...
    emitEvent(
        ...
        chat.members,
        {
            message:`${userThatWillBeRemoved.name} nikal diya bhai tujhe group se`,
            chatId
        }
    )
    emitEvent(
...
const deleteChat = TryCatch(async(req,res,next)=>{
    ...
    // now we have to delete chats and also attachments from cloduinary
